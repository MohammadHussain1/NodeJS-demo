...
export class AdvancedView extends LitElement {
    ...
    constructor() {
        ...
        // API Tier presets
        this.apiTierPresets = {
            free: { maxTokens: 60000, throttleAt: 70 },
            pro: { maxTokens: 1000000, throttleAt: 75 },
            enterprise: { maxTokens: 10000000, throttleAt: 80 }
        };
    ...
    // API Tier preset selection
    handleApiTierChange(e) {
        const tier = e.target.value;
        if (this.apiTierPresets[tier]) {
            const preset = this.apiTierPresets[tier];
            this.maxTokensPerMin = preset.maxTokens;
            this.throttleAtPercent = preset.throttleAt;
                    ...
                    <div class="form-grid">
                        ...
                        <div class="rate-limit-controls ${this.throttleTokens ? 'enabled' : ''}">
                            <div class="form-group" style="margin-bottom: 16px;">
                                <label class="form-label">API Tier Preset</label>
                                <select class="form-control" @change=${this.handleApiTierChange} ?disabled=${!this.throttleTokens}>
                                    <option value="pro" selected>Pro (1M tokens/min - Default)</option>
                                    <option value="free">Free (60K tokens/min)</option>
                                    <option value="enterprise">Enterprise (10M tokens/min)</option>
                                </select>
                                <div class="form-description">Select your API tier to automatically configure rate limiting settings</div>
                            </div>
