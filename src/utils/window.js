...
let intendedWindowSize = { width: 800, height: 1300 }; // Track intended size to prevent drift
...
function getDefaultKeybinds() {
    ...
    return {
        ...
        restartSession: isMac ? 'Cmd+Alt+R' : 'Ctrl+Alt+R',
    };
}
...
function updateGlobalShortcuts(keybinds, mainWindow, sendToRenderer, geminiSessionRef) {
        ...
        try {
            globalShortcut.register(keybinds.nextStep, async () => {
                ...
                try {
                    ...
                    // First check if cheddar.handleShortcut exists
                    console.log('Checking if cheddar.handleShortcut exists...');
                    const hasHandleShortcut = await mainWindow.webContents.executeJavaScript(`
                        typeof window.cheddar !== 'undefined' && typeof window.cheddar.handleShortcut === 'function';
                    `);
                    console.log('cheddar.handleShortcut exists:', hasHandleShortcut);
                    
                    if (hasHandleShortcut) {
                        // Use the new handleShortcut function
                        console.log('Calling cheddar.handleShortcut with:', shortcutKey);
                        mainWindow.webContents.executeJavaScript(`
                            cheddar.handleShortcut('${shortcutKey}');
                        `);
                    } else {
                        // Fallback to direct capture
                        console.log('Fallback: Directly calling captureManualScreenshot');
                        mainWindow.webContents.executeJavaScript(`
                    ...
                    // Additional fallback: send IPC message
                    sendToRenderer('take-manual-screenshot');
                } catch (error) {
    ...
    if (keybinds.emergencyErase) {
        try {
            globalShortcut.register(keybinds.emergencyErase, () => {
                console.log('Emergency erase shortcut triggered');
                if (mainWindow && !mainWindow.isDestroyed()) {
                    // Hide window immediately
                    mainWindow.hide();

                    // Clear sensitive data from renderer
    ...
    // Register restart session shortcut
    if (keybinds.restartSession) {
        try {
            globalShortcut.register(keybinds.restartSession, () => {
            ...
            console.log(`Registered restartSession: ${keybinds.restartSession}`);
        } catch (error) {
            console.error(`Failed to register restartSession (${keybinds.restartSession}):`, error);
        }
    }
...
function setupWindowIpcHandlers(mainWindow, sendToRenderer, geminiSessionRef) {
    ...
    ipcMain.handle('update-sizes', async event => {
        try {
            ...
            const baseHeight = layoutMode === 'compact' ? 800 : 800;
