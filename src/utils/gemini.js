...
async function exponentialBackoff(fn, retries = 3, baseDelay = 1000) {
    for (let i = 0; i <= retries; i++) {
        try {
            ...
            if (result && (!result.error || !result.error.includes('rate limit'))) {
                return result;
            }
            ...
            console.log(`[GEMINI] Rate limit hit. Retrying in ${Math.round(delay)}ms... (attempt ${i + 1}/${retries + 1})`);
            ...
            // Calculate delay with exponential backoff and jitter
            const delay = Math.min(baseDelay * Math.pow(2, i), 10000) + Math.random() * 1000;
            console.log(`[GEMINI] Request failed. Retrying in ${Math.round(delay)}ms... (attempt ${i + 1}/${retries + 1})`);
            await new Promise(resolve => setTimeout(resolve, delay));
        }
    }
}
...
function setupGeminiIpcHandlers(geminiSessionRef) {
    ...
    ipcMain.handle('initialize-gemini', async (event, apiKey, customPrompt, profile = 'interview', language = 'en-US', mode = 'interview', model = 'gemini-2.5-flash') => {
        // Use exponential backoff for initialization to handle rate limits
        const session = await exponentialBackoff(async () => {
            return await initializeGeminiSession(apiKey, customPrompt, profile, language, false, mode, model);
        }, 3, 2000);
    ...
    ipcMain.handle('send-image-content', async (event, { data, debug, isManual }) => {
        ...
        // Use exponential backoff for image sending to handle rate limits
        try {
    ...
    ipcMain.handle('send-screenshot-with-text', async (event, { imageData, text }) => {
        ...
        // Use exponential backoff for screenshot + text sending to handle rate limits
        try {
            return await exponentialBackoff(async () => {
